...
> **버전**: 1.1.0 | **작성일**: 2025-12-09 | **상태**: Draft
...
#### 2.1.4 UI Blocks (UI 블럭)

| 블럭 | 책임 | 파일 수 | 토큰 한도 |
|------|------|---------|----------|
| **BLOCK_FRONTEND** | GUI 모니터링 대시보드, 실시간 동기화 상태 표시 | 40개 | 45K |

> **상세**: [LLD: Frontend Monitoring](./lld/08_FRONTEND_MONITORING.md)
...
### 3.7 BLOCK_FRONTEND (프론트엔드 블럭)

```
┌─────────────────────────────────────────────────────────────────────┐
│                         BLOCK_FRONTEND                               │
├─────────────────────────────────────────────────────────────────────┤
│  책임: GUI 모니터링 대시보드, 실시간 동기화 상태 표시                  │
│                                                                      │
...
│  │   │   ├── dashboard/        # 대시보드 (DashboardStats, etc.)    │
│  │   │   ├── sync/             # 동기화 (SyncStatusCard, etc.)      │
│  │   │   └── common/           # 공통 UI (Card, Badge, etc.)        │
│  │   ├── hooks/                # Custom Hooks                        │
│  │   │   ├── useWebSocket.ts   # WebSocket 연결                      │
│  │   │   └── useSyncStatus.ts  # 동기화 상태 관리                    │
│  │   ├── store/                # Zustand 상태 관리                   │
│  │   │   ├── syncStore.ts      # 동기화 상태                         │
│  │   │   └── dashboardStore.ts # 대시보드 통계                       │
│  │   ├── services/             # API 클라이언트                       │
│  │   └── types/                # TypeScript 타입 정의                │
│  └── tests/                                                          │
│                                                                      │
│  입력: WebSocket events, REST API responses                          │
│  출력: UI 렌더링, 사용자 인터랙션 처리                                │
│                                                                      │
│  의존: BLOCK_SYNC (이벤트 구독), Backend API (REST + WebSocket)      │
│                                                                      │
│  ⚠️ 격리 규칙:                                                       │
│  - 허용: /frontend/** 내 파일만 접근 가능                            │
│  - 금지: /backend/**, /blocks/**, *.sql, *.py                        │
│  - 통신: REST API, WebSocket 통해서만 데이터 교환                    │
└─────────────────────────────────────────────────────────────────────┘
```

**FrontendAgent Capabilities (수행 가능 액션):**

| 액션 | 입력 | 출력 | 설명 |
|------|------|------|------|
| render_dashboard | - | JSX | 대시보드 메인 화면 렌더링 |
| handle_websocket | WsMessage | void | WebSocket 이벤트 처리 |
| update_sync_status | SyncStatus | void | 동기화 상태 UI 업데이트 |
| show_notification | Notification | void | 알림 토스트 표시 |
| fetch_stats | - | DashboardStats | API에서 통계 조회 |
| trigger_sync | 'nas' \| 'sheets' | void | 수동 동기화 트리거 |
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| 1.1.0 | 2025-12-09 | BLOCK_FRONTEND 추가 (섹션 3.7), FrontendAgent 정의, 블럭 격리 원칙 명시 |
